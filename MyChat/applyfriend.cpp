    ...
    , ui(new Ui::ApplyFriend), _label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("make1tRight"));
    ui->lb_ed->setPlaceholderText("search, add tags");
    ui->back_ed->setPlaceholderText("hellocpp");
    ...
    _tip_data = { "classmates","family","C++ Primer","Rust Programming",
                 "Python","go Programming", "WeChat" };
...
void ApplyFriend::AddTipLbs(ClickedLabel* lb, QPoint cur_point, QPoint &next_point, int text_width, int text_height)
{
    ...
    next_point.setX(lb->pos().x() + text_width + 15);//下一个节点往后加
