...
int MysqlDao::RegUser(const std::string& name, const std::string& email, const std::string& pwd) {
    ...
    try {
        ...
        stmt->execute();
        // PreparedStatement不支持注册输出参数, 需要使用会话变量或其他方法获取输出参数的值
        // 如果存储过程设置了会话变量, 或有其他方式获取输出参数的值, 可以在这里执行SELECT查询来获取他们
...
bool MysqlDao::UpdatePwd(const std::string& name, const std::string& newpwd) {
    ...
    try {
        ...
        //绑定参数
        pstmt->setString(2, name);      //第二个问号
        pstmt->setString(1, newpwd);    //第一个问号
        //执行UPDATE
